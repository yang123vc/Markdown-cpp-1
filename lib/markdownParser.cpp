...
void MarkdownParser::code_block(list<string>::iterator& line, int level, list<string>::iterator ende)
{
  ...
  string language = m_match[1];
  bool is_codeblock = false;
  do
  {
    ++line;
    if( iscodeblock_end(*line))
    {
      string end = m_match[1];
      if( !end.empty())
        tempStrings.push_back(end);
      is_codeblock = true;
      break;
    }
    else if( line == ende)
  ...
  if(is_codeblock)
  {
    ...
    code_begin_event(language);
    string front = tempStrings.front();
    tempStrings.pop_front();
    front = front.substr( 3 + language.length());
    if( front.front() == ' ')
      front = front.substr(1);
    if( !front.empty())
      tempStrings.push_front(front);

    for(string s : tempStrings)
    {
      for( size_t j = 0; j < s.length(); ++j)
        replace_code_char(s[j]);
      insert_line();
    }
  ...
  {
    list<string>::iterator it = tempStrings.begin();
    paragraph_block( it, level, tempStrings.end());
  }
}
...
void MarkdownParser::table_block(list<string>::iterator& line, int level, list<string>::iterator ende)
{
  string header = (*line);
  ++line;
  if( istable_align(*line))
  {
    string config = (*line);
    ...
    first = true;
    string name = "";
    table_begin_event(table_header);
    level++;
    insert_level(level);
    table_begin_row();
    level++;
    ...
    for( string content_line : tableContent)
    {
      ...
      regex exp("\\|\\|+");
      smatch e_match;
      while( regex_search(content_line, e_match, exp))
      {
        char buffer[2];
        buffer[0] = (char)e_match.length();
        buffer[1] = '\0';
        content_line.replace(e_match.position(), e_match.length(), buffer);
      }
      ...
      for( char c : content_line)
      {
        ...
        if( c == '|' || c < '\r')
        {
          size_t pos = temp_name.find_last_not_of(" \t");
          if( pos != string::npos)
            temp_name = temp_name.substr(0, pos+1);
          pos = temp_name.find_first_not_of(" \t");
          if( pos != string::npos)
            temp_name = temp_name.substr(pos);
          insert_level(level);
          if( c == '|')
            table_begin_col();
          else
            table_begin_multi_col((int)c);
          parse_line(temp_name);
          if(c == '|')
            table_end_col();
          else
            table_end_multi_col();
  ...
  {
    --line;
    paragraph_block(line, level, ende);
  }
}
...
bool MarkdownParser::iscodeblock_end(string& line)
{
  return regex_search(line, m_match, m_code_end);
}

bool MarkdownParser::istable(string& line)
{
  return (regex_search(line, m_match, m_table) || regex_search(line, m_match2, m_table_empty));
}
...
int MarkdownParser::find_inline_code_block(string& s)
{
  size_t len = 0;
  ...
  if( regex_search(s, m_match, m_inline_code_block))
  {
    ...
    preformat_begin_event();
    code_begin_event(m_match.str(2));
